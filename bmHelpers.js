...
module.exports={
    ...
    bhotm:{
        getEntryType: function(link){ //Using a link, determine the entry type. Return object, format and link.
            ...
            if (link == ""){ //No link means ur mason
                format = "mason";
            }
            else if(link.includes("youtube.com")){ //If the link is youtube, convert it to an embed format
                format = "youtube";
            ...
            else if(link.includes(" ")){ // If the link has a space, it's a phrase
                format = "phrase";
            }
        ...
        processMonth: function(month){ //Process a bhotm month from a form, sort the entries, set winner flags and entry formats
        ...
        dueMoment: function(incJ){  //Returns next due date moment
            ...
            if((moment().get("date") > day)){ //if it's after the 4th, add a month and set submission time
        ...
        judgeMoment: function(){ //Returns next moment when judging has ended
            return(this.dueMoment(true).add(3, "d"));
        },
        
        currentMonth: function(){ //Returns format of the current month entries fall in
            return(this.dueMoment(true).format("MMM. YYYY"));
        },

        canSubmit: function(){    //Returns true if we are not in the judging period
